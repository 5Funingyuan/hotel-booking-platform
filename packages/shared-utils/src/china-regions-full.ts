// 中国行政区划数据 - 省市二级（完整版，约340+城市）
// 不包含区县级数据以保持轻量，适合移动端使用
...
const provinceMeta: Array<{ code: string; name: string }> = [
...
export const allCities: City[] = [
  ...
  // F
  { code: "130600", name: "保定市", pinyin: "baoding", letter: "F" },
  { code: "130800", name: "承德市", pinyin: "chengde", letter: "F" },
  { code: "210400", name: "抚顺市", pinyin: "fushun", letter: "F" },
  { code: "210900", name: "阜新市", pinyin: "fuxin", letter: "F" },
  { code: "220100", name: "长春市", pinyin: "changchun", letter: "F" },
  { code: "341200", name: "阜阳市", pinyin: "fuyang", letter: "F" },
  { code: "350100", name: "福州市", pinyin: "fuzhou", letter: "F" },
  { code: "361000", name: "抚州市", pinyin: "fuzhou", letter: "F" },
  ...
  { code: "620700", name: "张掖市", pinyin: "zhangye", letter: "G" },
  ...
  // J
  { code: "130500", name: "邢台市", pinyin: "xingtai", letter: "J" },
  ...
  { code: "371200", name: "莱芜市", pinyin: "laiwu", letter: "L" },
  ...
  { code: "441200", name: "肇庆市", pinyin: "zhaoqing", letter: "Q" },
  ...
  { code: "341300", name: "宿州市", pinyin: "suzhou", letter: "S" },
  ...
  { code: "350200", name: "厦门市", pinyin: "xiamen", letter: "X" },
...
export const provinces: Province[] = provinceMeta.map((province) => {
  const prefix = province.code.slice(0, 2)
  const cities = allCities.filter((city) => city.code.startsWith(prefix))
...
export const getDistricts = (_provinceCode: string, cityCode: string): District[] => {
  if (!cityCode) return []
  return districtMap[cityCode] ?? []
}

// 按字母分组城市
export const groupedCities = allCities.reduce((acc, city) => {
...
// 字母列表（只包含有城市的字母）
export const alphabetLetters = Object.keys(groupedCities).sort()
...
// 保存最近访问
export const saveRecentCity = (cityName: string) => {
  if (typeof window === 'undefined') return
  try {
